      ...
      {showPermissionModal && (
        <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none">
          <div className="rounded-2xl shadow-2xl max-w-sm w-full p-6 space-y-4 pointer-events-auto text-center">
            <Youtube
              className="h-10 w-10 mx-auto text-foreground"
            />
...
export function Player() {
  ...
  const { addVideo, removeVideo, getVideosByTags } = useTag();
  ...
  const [useThumbnailBackground, setUseThumbnailBackground] = useState(() => {
    return localStorage.getItem('use-thumbnail-background') === 'true';
  });
  ...
  const [showAdvancedSettings, setShowAdvancedSettings] = useState(() => {
    const saved = localStorage.getItem('show-advanced-settings');
    return saved === 'true';
  });
  const [isAdvancedButtonHovered, setIsAdvancedButtonHovered] = useState(false);
  const [showSquareInfo, setShowSquareInfo] = useState(false);

  const isYouTubeType = playerType === 'normal' || playerType === 'youtube';

  const isInLibrary = currentVideoId
    ? getVideosByTags([]).some((v) => v.id === currentVideoId)
    : false;
  ...
  // Save advanced settings visibility to localStorage when it changes
  useEffect(() => {
    savePreference('show-advanced-settings', showAdvancedSettings);
  ...
  useEffect(() => {
    setThumbnailBackgroundUrl(
      currentVideoId ? getYouTubeThumbnailUrl(currentVideoId, 'hqdefault') : null
    );
  }, [currentVideoId]);
  ...
  // 
  // Toggle thumbnail background and save preference
  const toggleThumbnailBackground = () => {
    const newValue = !useThumbnailBackground;
    setUseThumbnailBackground(newValue);
    localStorage.setItem('use-thumbnail-background', String(newValue));
  };
  ...
  return (
    <div className="relative min-h-screen">
      {(useThumbnailBackground && thumbnailBackgroundUrl || customBackground) && (
        <div
          className="fixed inset-0"
          style={{
            backgroundImage: useThumbnailBackground && thumbnailBackgroundUrl
          ...
          <div className="space-y-8">
            ...
            >
              <div
                ...
                style={{
                  width: (() => {
                    const ratio = forceSquareRatio && isYouTubeMusicVideo ? 1 : (videoAspectRatio || 16/9);
                    return ratio < 16/9 ? `${ratio / (16/9) * 100}%` : '100%';
                  })(),
                  height: (() => {
                    const ratio = forceSquareRatio && isYouTubeMusicVideo ? 1 : (videoAspectRatio || 16/9);
                    return ratio > 16/9 ? `${(16/9) / ratio * 100}%` : '100%';
                  })(),
                }}
              >
                <div className="shadow-2xl overflow-hidden rounded-none sm:rounded-3xl w-full h-full bg-transparent">
                  {videoAspectRatio && (
                    <VideoPlayer
                      ...
                      autoPlayEnabled={autoPlayEnabled}
                      loopEnabled={loopEnabled}
                      forcedAspectRatio={forceSquareRatio && isYouTubeMusicVideo ? 1 : null}
                    />
                  )}
                </div>
                {isDragging && (
                  <div className="absolute inset-0 bg-foreground/20 flex items-center justify-center z-10 backdrop-blur-sm rounded-none sm:rounded-3xl">
                    <div className="text-center">
                      <p className="text-foreground text-lg font-semibold">Drop video ID here</p>
                      <p className="text-foreground/80 text-sm mt-1">YouTube URL or video ID</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="flex flex-col gap-4 mt-6">
                ...
                {isEditing ? (
                  ...
                  <div className="relative ml-4">
                    ...
                    >
                      ...
                      <div className="h-full pl-10 pr-3 flex items-center text-sm text-muted-foreground group-hover:text-foreground transition-colors truncate" style={{ fontFamily: "monospace" }}>
                        {currentVideoId || ''}
                      </div>
                    </div>
                  </div>
              ...
              {/* Row 3: Set Background, Square - Collapsible */}
              <div className="flex flex-col">
                <div
                  className="flex items-center justify-between transition-all duration-200"
                  style={{
                    maxHeight: showAdvancedSettings ? '100px' : '0',
                    opacity: showAdvancedSettings ? 1 : 0,
                    marginBottom: showAdvancedSettings ? '8px' : '0',
                    overflow: showAdvancedSettings ? 'visible' : 'hidden',
                  }}
                >
                {/* Left Group: Thumbnail BG */}
                <div className="flex items-center gap-4 flex-nowrap">
                  {/* Thumbnail Background Toggle */}
                  <button
                    onClick={toggleThumbnailBackground}
                    className="flex items-center gap-2 px-3 py-2 rounded-md transition-colors border-none cursor-pointer"
                    style={{
                      color: useThumbnailBackground
                        ? 'white'
                        : 'hsl(var(--muted-foreground))',
                    }}
                    onMouseEnter={(e) => {
                    ...
                    onMouseLeave={(e) => {
                      e.currentTarget.style.color = useThumbnailBackground
                        ? 'white'
                        : 'hsl(var(--muted-foreground))';
                    }}
                  >
                ...
                {/* Right Group: Cookies, Better Ratio */}
                <div className="flex items-center gap-4 flex-nowrap">
                  {/* Cookie Toggle */}
                  <button
                    ...
                    style={{
                      color: isYouTubeType && cookiesEnabled
                        ? 'white'
                        : 'hsl(var(--muted-foreground))',
                    }}
                    onMouseEnter={(e) => {
                  ...
                  {/* Square Ratio Toggle (Experimental) */}
                  <div className="relative">
                    <button
                      ...
                      onMouseEnter={(e) => {
                        e.currentTarget.style.color = 'hsl(var(--foreground))';
                        setShowSquareInfo(true);
                      }}
                      onMouseLeave={(e) => {
                        e.currentTarget.style.color = forceSquareRatio
                          ? 'white'
                          : 'hsl(var(--muted-foreground))';
                        setShowSquareInfo(false);
                      }}
                    >
                    ...
                    {showSquareInfo && (
                      <div
                        className="absolute right-0 bottom-full mb-2 z-50 bg-card/20 backdrop-blur-md border border-border rounded-lg shadow-lg p-3 w-64 text-sm"
                        style={{
                      ...
                      >
                        <div className="flex flex-col gap-2">
                          ...
                          <p className="text-muted-foreground leading-relaxed">
                            Force a square ratio on music videos for a more immersive experience
                          </p>
                        </div>
                      </div>
                    )}
                ...
                <button
                  ...
                  className="flex items-center justify-center px-3 rounded-md transition-colors border-none cursor-pointer"
                  style={{
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.color = 'hsl(var(--foreground))';
                    setIsAdvancedButtonHovered(true);
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.color = 'hsl(var(--muted-foreground))';
                    setIsAdvancedButtonHovered(false);
                  }}
                >
                  <div
                    ...
                    style={{
                      ...
                      width: isAdvancedButtonHovered || showAdvancedSettings ? '64px' : '32px',
                    }}
                  />
                </button>
              </div>
